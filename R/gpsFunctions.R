#' Transfer files from GPS to geospatialDataBackup folder
#' 
#' Find all job that are on the connected GPS unit and move to geospatialDataBackup.
#' It only works on Hal (and probably R2D2) unless your computer can connect to the GPS
#' 
#' @param machine which GPS machine (sulu or chek) that is being tranferred from
#' @param year the year that the GPS points were taken
#' @param user rarely change this but if using a computer other than Hal, change 
#'  so that the file path will work (e.g. "dhanson")
#' @keywords gps
#' 
#' @return The names of the files that were transferred from the GPS to
#'  geospatialDataBackup.
#'  
backupGPSFiles <- function(machine, year, user = "Echinacea") {
  jobPath <- "Computer/WM_swagenius/Program Files/TPS/TopSURV/Jobs"
  if (machine == "sulu") {
    backupPath <- paste("C:/Users/", user, "/Dropbox/geospatialDataBackup",
                        year, "/backupSuluJobs", year, sep="")
  } else if (machine == "chek") {
    backupPath <- paste("C:/Users/", user, "/Dropbox/geospatialDataBackup",
                        year, "/backupChekovJobs", year, sep="")
  } else {
    stop("job must be either \"sulu\" or \"chek\"")
  }
  
  if (!dir.exists(jobPath)) stop(paste("The GPS path you want doesn't exist: ", jobPath))
  if (!dir.exists(backupPath)) stop(paste("The backup path you want doesn't exist: ", backupPath))
  
  jobFiles <- list.files(jobPath)
  backupFiles <- list.files(backupPath)
  # remove all .bak files. We don't like/care about those
  jobFiles <- jobFiles[!grepl(".bak", jobFiles)]
  # get rid of all other files if there are any non gps ones
  jobFiles <- jobFiles[grepl(".tsj", jobFiles)]
  
  notInBackup <- setdiff(jobFiles, backupFiles)
  
  jobFilePaths <- paste(jobPath, notInBackup, sep = "/")
  backupFilePaths <- paste(backupPath, notInBackup, sep = "/")
  
  file.copy(jobFilePaths, backupFilePaths)
  message(paste("Copied ", length(notInBackup), " file(s) from ", machine, " to geospatialDataBackup", year, sep = ""))
  notInBackup
}
